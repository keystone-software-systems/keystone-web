...
import { addComment, type CommentActionState } from "@/actions/comments";
...
export function CommentThread({
  ...
  const boundAction = addComment.bind(null, projectId);
          ...
          {comments.map((comment) => (
            <li key={comment.id} className="rounded-md border border-slate/20 bg-white p-3">
              <p className="text-xs text-slate">
                {comment.author_id === currentUserId ? "You" : "Keystone Systems"} ·{" "}
