...
- **Bench depth (resolved 2026-07, scoped):** The founder has an established network of senior independent engineers — some full-time, some moonlighting alongside another role — he brings in when a project calls for it. **The operating model stays founder-led**: the founder personally scopes every engagement and decides who, if anyone, gets looped in. This is not a pivot to a dev-shop or marketplace model.
  - **Public marketing site (`apps/web`) is unchanged.** Keep the current generic framing ("a small network of equally experienced independent engineers") — no numbers, no names, founder-led judgment stays the headline. Do not add bench-depth specifics to any `apps/web` page without a separate, explicit decision.
  - **What did change:** a project intake portal is planned (`docs/intake-portal-design.md`, `apps/portal`) where a client submits a project and, once the founder assigns a specific network engineer to it, that client can see who's working on their project — name, background, relevant skills — inside their own authenticated portal dashboard. This is disclosure to a client with a live, matched engagement, not a public claim, and should be treated differently from anything on `apps/web`.
  - **Wording guardrail:** wherever an engineer's background is shown (portal or otherwise), state it plainly with specifics — prior companies where shareable, years of experience — never a superlative like "world-class." Same treatment as the founder's own Stripe/Microsoft line above: say the specific thing once, let it carry the weight.
